...
import { Calendar, Folder, Activity, ArrowUpRight } from "lucide-react";
...
export const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  const progress =
    totalColumns > 0 ? Math.round((completedColumns / totalColumns) * 100) : 0;
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-all duration-300 transform hover:-translate-y-1">
      ...
      <div className="p-6">
        <div className="flex justify-between items-start mb-4">
          <div>
            <span className="font-bold text-gray-900 text-lg mb-1 block">
              {project.name}
            </span>
            <span
        ...
        <Link
          to={`/${project.name}/${project.workspaceId}/projects/columns/${project.id}`}
          className={`flex items-center px-3 py-2 rounded-full mt-4 justify-center`}
          style={{
            backgroundColor: `${project.color}20`,
          }}
        >
